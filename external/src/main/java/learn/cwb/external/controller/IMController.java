...
public class IMController {
    ...
    public void work() {
        ...
        ChannelFuture channelFuture = serverBootstrap
                ...
                .bind("127.0.0.1", SystemConstant.IM_PORT)
                .syncUninterruptibly();
    }

    private static void refreshAvailableNodes() {
        ZookeeperOps zookeeperOps = new ZookeeperOpsImpl();
        List<String> nodes = zookeeperOps.getChild(SystemConstant.IM_NODE_PATH_PREFIX, IMController::watchedEvents);
    ...
    private static void watchedEvents(WatchedEvent event) {
        Set<String> now = new HashSet<>(ZOOKEEPER_OPS.getChild(SystemConstant.IM_NODE_PATH_PREFIX, IMController::watchedEvents));
    ...
    private static void setIMNode(String address) {
        ...
        ChannelFuture channelFuture = bootstrap
                ...
                .handler(new ChannelInitializer<>() {
                    ...
                    protected void initChannel(Channel ch) throws Exception {
                        ...
                        pipeline.addLast("DirectHandler", new IMHandler());
                    }
                })
    ...
    private static void delIMNode(String address) {
        Channel channel = SERVERS.get(address);
        channel.close();
        SERVERS.remove(address);
    }

    private static class IMHandler extends ChannelInboundHandlerAdapter {
    ...
    private static class KeepAliveHandler extends ChannelInboundHandlerAdapter {
        ...
        private static void channelRmv(Channel channel) {
            ...
            THREAD_POOL.execute(() -> {
                REDIS_OPS.delObj(SystemConstant.USER_IN_IM_CLUSTER_PREFIX + id);
            });
        }
        ...
        private static void channelAdd(Channel channel) {
            ...
            THREAD_POOL.execute(() -> {
                REDIS_OPS.setObj(SystemConstant.USER_IN_IM_CLUSTER_PREFIX + id, NativeUtils.myAddress(SystemConstant.IM_PORT));
            });
        }
    }
}
